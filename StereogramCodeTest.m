...
pattern_width = stripSize(2);
% pattern = imresize(pattern, [pattern_width pattern_width]);
pattern_ = [pattern_width size_y];
 
for j=1:pattern_width
    for i=1:size_y
        pattern_(i,j,1)=strip(rem(i,pattern_width)+1,j,1);
        pattern_(i,j,2)=strip(rem(i,pattern_width)+1,j,2);
        pattern_(i,j,3)=strip(rem(i,pattern_width)+1,j,3);
    end
end
