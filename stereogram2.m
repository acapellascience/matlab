function stereoImage = stereogram2(DepthMapFile,StripFile);
...
%In this version, we are laying down two altered initial strips side by
%side, then continuing from there as before.

%Left Strip
for i=1:stripSize_x
    
    x_position = mapCenter_x+1-i;
    
    for j = 1:stripSize_y
        
        z = int64(floor(double(map(j,x_position+stripCenter_x))*factor*(i-1)/stripSize_x/(1-double(map(j,x_position+stripCenter_x))*factor/stripSize_x)));
        
        if stripSize_x+1-i-z >= 1 && stripSize_x+1-i-z <= stripSize_x
            
            stereoImage(j,x_position,1) = strip(j,stripSize_x+1-i-z,1);
            stereoImage(j,x_position,2) = strip(j,stripSize_x+1-i-z,2);
            stereoImage(j,x_position,3) = strip(j,stripSize_x+1-i-z,3);
...
for i=1:stripSize_x
    ...
    x_position = mapCenter_x+i;
    
    for j = 1:stripSize_y
        
        z = int64(ceil(double(map(j,x_position+stripCenter_x-stripSize_x))*factor*(i-1)/stripSize_x/(1-double(map(j,x_position+stripCenter_x-stripSize_x))*factor/stripSize_x)));
        
        if i+z >= 1 && i+z <= stripSize_x
            
            stereoImage(j,x_position,1) = strip(j,i+z,1);
            stereoImage(j,x_position,2) = strip(j,i+z,2);
            stereoImage(j,x_position,3) = strip(j,i+z,3);
